        ...
        if srl_layer is None:
            ...
            for predicate in predicates:
                ...
                for role in roles:
                    ...
                    frames = getAnnotations(role, mentions)
    ...
    for wf in text_layer.getchildren():
        for s in spans:
            ...
            if wf.get('id') == s.attrib.get('id'):
                token = {"token_id": wf.get('id'), 'sent': wf.get('sent'), 'offset': wf.get('offset'),
                ...
                for component in components:
                    if component.get('id') == s.attrib.get('id'):
                        token = {"token_id": component.get('id'), 'sent': wf.get('sent'),
...
def getAnnotations(predicate, mentions):
    ...
    for ref in frame_info:
        ...
        annotation = {'project': 'DutchFrameNet', 'status': status, 'annotator': source, 'timestamp': t,
                      'reftype': reftype, 'mention': []}
